...
def write_comparison_html(df, outdir):
    """Write a comparison dashboard for multiple designs."""
    os.makedirs(outdir, exist_ok=True)
    styled = df.style.format({"Design": lambda x: make_clickable(x, x)})
...
# ----------------------------
# Report discovery (Timing only)
# ----------------------------
def find_reports(cwd, design, design_name):
    """Discover timing reports. Removed power report logic."""
    ...
    for scen in sorted(os.listdir(design_dir)):
        ...
        scen_dir = os.path.join(design_dir, scen)
...
def parse_timing(report_path):
    ...
    return ([setup["WNS"], setup["TNS"], setup["NUM"]],
            [hold["WNS"], hold["TNS"], hold["NUM"]])
...
# ----------------------------
# Histogram generation (Timing only)
# ----------------------------
...
# ----------------------------
# DataFrame generation (Timing only)
# ----------------------------
...
def compute_baseline_deltas(df, baseline_design, metrics):
    ...
    for col in metrics:
        ...
        df[delta_col] = df.apply(
            lambda r: (r[col] - baseline.loc[r["Scenario"], col]) 
            if r["Scenario"] in baseline.index and r["Design"] != baseline_design 
            else (0.0 if r["Design"] == baseline_design else None), axis=1)
    return df

# ----------------------------
# Main entry (Timing only)
# ----------------------------
def run_chip(cwd, die, outdir, designs):
    ...
    for design_folder in designs:
        ...
        for h_name, h_df in hists.items():
            h_styled = h_df.style.format({"Design": lambda x: make_clickable(x, x), 
                                          "Die": lambda x: make_clickable(x, x)})
    ...
    # comparison dashboard if multiple designs
    if len(all_raw) > 1:
